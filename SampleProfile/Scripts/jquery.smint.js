		...
		return $('.smint li').each( function(index) {
			...
			var stickyMenu = function(direction){
				...
				if (scrollTop > stickyTop) { 
				    ...
				    $('.smint').css({ 'position': 'absolute', 'top': 70 }).removeClass('fxd');
				}   

				//Check if the position is inside then change the menu
				// Courtesy of Ryan Clarke (@clarkieryan)
				var pos1 = $("div." + id).position().top - menuHeight;
			    var pos2 = $("div." + id).height() + $("div." + id).position().top;

				if(pos1 <= scrollTop && scrollTop <= pos2){	
					if(direction == "up"){
						$("#"+id).addClass("active");
